...
public class CopyOnWriteArrayList<E>
     ...
     * @param index first index to search  搜索开始索引
     * @param fence one past last index to search 搜索结束索引
     * @return index of element, or -1 if absent
     */
    private static int indexOf(Object o, Object[] elements,
                               int index, int fence) {
    	//目录element为null,遍历是否在在null
        if (o == null) {
    ...
    public int addAllAbsent(Collection<? extends E> c) {
        ...
        try {
            ...
            for (int i = 0; i < cs.length; ++i) { // scan for duplicates
                Object e = cs[i];
                if (indexOf(e, elements, 0, len) < 0 &&
                    indexOf(e, uniq, 0, added) < 0)   //检查自身重复
                    uniq[added++] = e;
            }
            //需要添加
            if (added > 0) {
                Object[] newElements = Arrays.copyOf(elements, len + added);//Copy一个数组(长度为扩展后长度)
                System.arraycopy(uniq, 0, newElements, len, added); //本地方法
                setArray(newElements);
            }
            return added;
        } finally {
