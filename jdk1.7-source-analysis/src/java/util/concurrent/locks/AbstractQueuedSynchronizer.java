 ...
 * }
 * </pre>
 *  public abstract class AbstractQueuedSynchronizerextends AbstractOwnableSynchronizerimplements
 *  Serializable为实现依赖于先进先出 (FIFO) 等待队列的阻塞锁和相关同步器（信号量、事件，等等）提供一个框架。此类的设计目标是成为依靠单个原子 int 值来表示状态的大多数同步器的一个有用基础。子类必须定义更改此状态的受保护方法，并定义哪种状态对于此对象意味着被获取或被释放。假定这些条件之后，此类中的其他方法就可以实现所有排队和阻塞机制。子类可以维护其他状态字段，但只是为了获得同步而只追踪使用 getState()、setState(int) 和 compareAndSetState(int, int) 方法来操作以原子方式更新的 int 值。 
...
public abstract class AbstractQueuedSynchronizer
    ...
    private Node enq(final Node node) {
        for (;;) {
            Node t = tail;
            if (t == null) { // Must initialize  // tail节点为空，说明是空队列，初始化头节点，如果成功，返回头节点
     ...
     *  (1)如果tail节点不为null，说明队列不为空，则把新节点加入到tail的后面，返回当前节点，否则进入enq进行处理(2)；
        (2)如果tail节点为null，说明队列为空，需要建立一个虚拟的头节点，并把封装了当前线程的节点设置为尾节点；另外一种情况的发生，
                                  是由于在(1)中的compareAndSetTail可能会出现失败，这里采用for的无限循环，是要保证当前线程能够正确进入等待队列；
     */
    private Node addWaiter(Node mode) {
        ...
        if (pred != null) {
            node.prev = pred;
            if (compareAndSetTail(pred, node)) { //当前结点设置为tail结点
                pred.next = node;
                return node;
            }
        }
    ...
    private void unparkSuccessor(Node node) {
         ...
         */
        Node s = node.next;                       //当前执行线程节点的下一个节点
        if (s == null || s.waitStatus > 0) {      //s为null或者等待状态>0,从后向前遍历线程结点，如果线程结点等待状态<=0
            s = null;
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws > 0) {
             ...
             * indicate retry.  1 1 1 0 0 0 - > 000000
             */
            do {
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
     ...
     * acquire分析
		(1)如果当前节点是队列的头结点（如果第一个节点是虚拟节点，那么第二个节点实际上就是头结点了），就尝试在此获取锁tryAcquire(arg)。
		       如果成功就将头结点设置为当前节点（不管第一个结点是否是虚拟节点），返回中断状态。否则进行(2)。 
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {   
                ...
                //第一个节点是虚拟节点，那么第二个节点实际上就是头结点
                if (p == head && tryAcquire(arg)) { //当前结点头结点tryAcquire
                    setHead(node);
                    p.next = null; // help GC
                    failed = false;
                    return interrupted;
    ...
    public final void acquire(int arg) {
    	//如果尝试以独占的方式获得锁失败，那么就把当前线程封装为一个Node，加入到等待队列中；如果加入队列成功，接下来检查当前线程的节点是否应该等待(挂起)，
    	//如果当前线程所处节点的前一节点的等待状态小于0，则通过LockSupport挂起当前线程；
