...
public class MemoryHandler extends Handler {
    ...
    public MemoryHandler() {
        ...
        try {
            ...
            Class clz = ClassLoader.getSystemClassLoader().loadClass(name); //系统类加载器加载类，并调用类newInstance实例化类
            target = (Handler) clz.newInstance();
        } catch (Exception ex) {
            throw new RuntimeException("MemoryHandler can't load handler \"" + name + "\"" , ex);  //\"转义"，不是文件结束
        }
        init();
    }
    ...
    public synchronized void publish(LogRecord record) {
        ...
        //值超过了内存缓冲区（数组容量）
            ...
            start++;
            start %= buffer.length;  //从数组开头可始循环
        }
        if (record.getLevel().intValue() >= pushLevel.intValue()) {  //level
            push();
        }
    }
    ...
    public synchronized void push() {
        for (int i = 0; i < count; i++) {
            int ix = (start+i)%buffer.length;   //count =30; start=4 buffer =20  循环buffer    ?
