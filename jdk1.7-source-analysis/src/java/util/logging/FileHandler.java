 ...
 * 
 * 
 *  java.util.logging.FileHandler.pattern = ./logs/info.log
	java.util.logging.FileHandler.limit = 1000
	java.util.logging.FileHandler.count = 20000
...
public class FileHandler extends StreamHandler {
    ...
    private void openFiles() throws IOException {
        ...
        // to our set of output files, as long as we are alive. 创建一个锁文件，授权一个排他的访问文件
        int unique = -1;
        for (;;) {
            unique++;
            if (unique > MAX_LOCKS) {
  ...
  */
    ...
    private synchronized void rotate() {
        ...
        for (int i = count-2; i >= 0; i--) {
            File f1 = files[i];   //8 7 6
            File f2 = files[i+1]; //9 8 7
            if (f1.exists()) {
                if (f2.exists()) {
                    f2.delete();
                }
                f1.renameTo(f2);  //f1重命名f2文件名
            }
        }
        //再重新打开文件
        try {
