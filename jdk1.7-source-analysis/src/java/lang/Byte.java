...
public final class Byte extends Number implements Comparable<Byte> {
    ...
    /**
     * 字节数组缓存 （静态内部类不需要外部成员，只能访问外部类的静态成员)
     */
    private static class ByteCache {
        private ByteCache(){}

        static final Byte cache[] = new Byte[-(-128) + 127 + 1]; //257个字符

        static {
            for(int i = 0; i < cache.length; i++)     
                cache[i] = new Byte((byte)(i - 128)); //值:-128-127
        }
    }
     ...
     * @since 1.5
     */
    public static final int SIZE = 8;  // 用于以二进制补码形式表示 byte 值的位数。
